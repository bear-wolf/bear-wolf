    ...
    return this.each(function () {
        ...
        function rotate(Current, $active) {
            ...
            if (_border) {
                ...
                if (Direction < 0) {
                    if (mnoj == 0) return;
                    mnoj = Math.abs(mnoj) - _el_count_length;
                }

                if ($active.attr("rel") >= _slide) {
                    if (ArrowOffset+1 >= All) return;
                    if (Direction > 0) mnoj = Math.abs(mnoj) + _el_count_length;
                }
        ...
        //On Click
        $(_findPagerClass).find("a").click(function () {
            $active = $(this);
            ReadData(_Current);
            rotate(_Current, $active); //Trigger rotation immediately		
        ...
        $(NAL).click(function () {
            ...
            /*if (CheckClickSecond(t)) { GetState(t) }
            else SaveState(t);*/
        ...
        // Check the number of clicks in 1 second
        //CheckClickSecond = function (t) {
        //    var r = false;
        //    var c = $(point + t.parent().data("base"));

        //    var Time = new Date();
        //    var GetTime = c.data("click-lock").split(' ');
        //    if ((parseInt(GetTime[0]) != -1) && parseInt(GetTime[0]) <= Time.getSeconds()) { r = true; }
        //    return r;
        //}

        //GetState = function (t) {
        //    var c = $(point + t.parent().data("base"));
        //    var pn = c.find(".pagination a");
        //    var obj = c.data("object").split(" ");
        //    pn.removeClass();
        //    pn.eq(obj[1] - 1).addClass(Active);
        //    $active = pn.eq(obj[1] - 1);
        //    c.find(_el).removeClass(Active);
        //    c.find(_el).eq(obj[0] - 1).addClass(Active);

        //    c.data("click-lock", "0 0");
        //}

        //SaveState = function (t) {
        //    var c = $(point + t.parent().data("base"));
        //    var Time = new Date();
        //    c.data("click-lock", Time.getSeconds() + " 0");

        //    // element 'slide' 'pager'
        //    c.data("object", (c.find(_el).filter(point + Active).index() + 1) + " " + (c.find(point + PagerClass + " a." + Active).index() + 1));
        //    return;
        //}        
    });
}
